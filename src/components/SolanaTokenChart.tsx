...
const SolanaTokenChart = ({
  ...
  const getPairsForToken = async (mint: string) => {
    ...
    const pairs = (data.pairs || []).filter((p: any) => p.chainId === "solana").sort((a: any, b: any) => (b.liquidity?.usd || 0) - (a.liquidity?.usd || 0));
    return pairs;
  };
  const getCandles = async (pairId: string, resolution: string, fromSec: number, toSec: number) => {
    const url = `https://api.dexscreener.com/latest/dex/candles/${encodeURIComponent(resolution)}/${encodeURIComponent(pairId)}?from=${fromSec}&to=${toSec}`;
    const res = await fetch(url);
  ...
  const loadAndRender = async (resolution: string) => {
    try {
      ...
      const {
        from,
        to
      } = rangeFromDays(lookbackDays);
      const raw = await getCandles(pairId, resolution, from, to);
      const seriesData = raw.map((c: any) => ({
  ...
  return <div className="w-full bg-white border border-gray-200 rounded-2xl p-4 shadow-sm">
      <div className="flex items-center justify-between gap-3 mb-3">
        ...
        <div className="flex items-baseline gap-2">
          ...
          <span className={`text-xs px-2 py-1 rounded-lg border ${priceChange && priceChange > 0 ? "bg-emerald-50 text-emerald-900 border-emerald-200" : priceChange && priceChange < 0 ? "bg-red-50 text-red-900 border-red-200" : "bg-white text-gray-700 border-gray-200"}`}>
            {priceChange !== null && Number.isFinite(priceChange) ? `${priceChange > 0 ? "+" : ""}${priceChange.toFixed(2)}%` : "—"}
          </span>
        </div>
      </div>

      <div className="flex gap-1.5 bg-gray-50 border border-gray-200 p-1.5 rounded-xl mb-3">
        {resolutions.map(res => <button key={res} onClick={() => setCurrentResolution(res)} className={`px-3 py-1.5 rounded-lg border text-sm transition-colors ${currentResolution === res ? "bg-gray-900 text-white border-gray-900" : "bg-white text-gray-700 border-gray-200 hover:bg-gray-100"}`}>
            {res}
          </button>)}
      </div>

      <div ref={chartContainerRef} className="w-full border border-gray-200 rounded-xl overflow-hidden bg-white" style={{
      height: "460px"
    }} />
      ...
      <div className="flex justify-between items-center mt-2 text-xs text-gray-500">
        <div>
          Data via{" "}
          <a href="https://dexscreener.com" target="_blank" rel="noreferrer" className="hover:underline">
            DexScreener
          </a>
        </div>
